...
export class Init1780148228337 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "email_confirmation" ("id" SERIAL NOT NULL, "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP WITH TIME ZONE, "expirationDate" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT '"2026-05-30T14:37:10.981Z"', "confirmationCode" uuid NOT NULL DEFAULT '1f27dc16-1701-49da-bc45-aa2735ef7698', "isConfirmed" boolean NOT NULL DEFAULT false, "userId" integer NOT NULL, CONSTRAINT "REL_28d3d3fbd7503f3428b94fd18c" UNIQUE ("userId"), CONSTRAINT "pk_email_confirmation_id" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE TABLE "recovery" ("id" SERIAL NOT NULL, "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP WITH TIME ZONE, "expiredAt" TIMESTAMP WITH TIME ZONE NOT NULL, "code" uuid NOT NULL DEFAULT 'dcab526f-1762-4170-9333-6528aefa4846', "userId" integer NOT NULL, CONSTRAINT "REL_318d006fbaa2a2aa666c3af387" UNIQUE ("userId"), CONSTRAINT "pk_recovery_id" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE TABLE "security_device" ("id" SERIAL NOT NULL, "createdAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP WITH TIME ZONE, "deviceName" character varying NOT NULL, "deviceId" uuid NOT NULL DEFAULT '8a257b1a-f530-4ff9-883a-86e687b485be', "expiredAt" TIMESTAMP WITH TIME ZONE NOT NULL, "ip" character varying, "issuedAt" TIMESTAMP WITH TIME ZONE NOT NULL, "userId" integer NOT NULL, CONSTRAINT "pk_security_device_id" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
